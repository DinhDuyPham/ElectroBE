...
exports.addProductToCart = async (req, res) => {
    try {
        ...
        const cartItemsActive = await CartItem.find({cart_id: cart.id, is_active: true});
        ...
        if (cartItemsActive.length > 0) {
            cart.total_item_active = cartItemsActive.length;
            cart.total_price_active = cartItemsActive.reduce((acc, curr) => acc + curr.total_price, 0);
            await cart.save();
        }
    ...
    try {
        ...
        if (listItem && Array.isArray(listItem)) {
            const cartItems = await Promise.all(listItem.map(async (item) => {
                const { id, qty, is_active } = item;
                const product = await Product.findById(id);
                const price = product.price;

                const cartItem = await CartItem.findOne({cart_id: cart.id, product_id: id});
                if (cartItem) {
                    cartItem.qty = qty > 0 ? qty : 1;
                    cartItem.total_price = price * cartItem.qty;
            ...
            const listCartItemsActive = await CartItem.find({cart_id: cart.id, is_active: true});
            cart.total_item = listCartItems.length;
            cart.total_price = listCartItems.reduce((acc, curr) => acc + curr.total_price, 0);
            cart.total_item_active = listCartItemsActive.length;
            cart.total_price_active = listCartItemsActive.reduce((acc, curr) => acc + curr.total_price, 0);
            await cart.save();
        }
...
exports.getCart = async (req, res) => {
    try {
        ...
        // await CartItem.updateMany({ cart_id: cart._id, is_active: true }, { is_active: false });
        ...
        cartItems.forEach(ci => {
            const attributeMap = attributeValueMap[ci.product_id] || {};
            Object.assign(ci, attributeMap);
        });
